...
for i in df.index:
    id=df['news_id'][i]
    sheet1.write(i+1,0,id)
    try:
        inputText= df['message'][i]
        if(len(inputText) > 1000):
                continue
        tranText = translator.translate(inputText, dest='en')
        sheet1.write(i+1,1,tranText.text)
        analysis=s.sentiment(tranText.text)
        sheet1.write(i+1,2,analysis[0])
        sheet1.write(i+1,3,analysis[1])
        
        
    except:
        #sheet1.write(i+1,1,'unicode error')
        continue

wb.save('sentimentAnalysis.xls')
